...
## Owner transfer (WP5D-4, ✅ accepted)

The offline `owner-transfer` CLI performs only DB-level canonical IP→IP ownership transfer. The supplied complete real PG16+age `pnpm verify:release` success evidence includes the real PostgreSQL owner-transfer gate, the real age gate, and the compiled + installed-npm PostgreSQL E2E smoke, all passing. The scope remains unchanged: policy-file IP entries, token bindings, and roles do not migrate; admin cross-owner read is not implemented; workspace security remains deferred until public exposure, and public exposure is prohibited. The tool has never run against real user data and requires explicit operator authorization for a real target. No test counts are recorded. WP5 as a whole remains incomplete: WP5B is DEFERRED and the WP5C deployment drill is not accepted.
...
WP5A adds two probe endpoints to the running service, plus the injectable process status backing them. **Requests to probes are IP-gated first, like every other route** (the global `onRequest` network admission covers `/health`, `/readyz` and `/metrics`; outside-CIDR / disabled / unparseable socket IP → `403` before any probe logic runs). Then: `/health` and `/readyz` accept **any admitted role** and **never require a token** (liveness/readiness must not be blocked by token issues — the token gate covers `/v1` and `/metrics` only); `/metrics` is role-gated **admin/operator only** (WP5D-3), and when that profile is `tokenRequired` the **actual `GET` still requires the Bearer token** — the sole exception is a compliant browser CORS preflight (`OPTIONS` + `Origin` + `Access-Control-Request-Method`), which is token-exempt on every path and then evaluated by the CORS origin policy. The unified WP5D-1/WP5D-2/WP5D-3 status is **✅ accepted**, based on the user's supplied complete real PG16+age `pnpm verify:release` success evidence from a new release run after `workspaceRoots`/`PI_DEFAULT_WORKSPACE_ROOT` were removed and commit `5e84a9da` was submitted (see [ip-rbac-design.md](ip-rbac-design.md)); IP-RBAC does **not** restrict cwd or Agent-tool absolute paths/OS permissions and is **not** a sandbox. Workspace/sandbox security is deferred until public exposure (future OIDC/IAM design); admin cross-owner read remains unimplemented, while **WP5D-4 owner transfer (DB-only IP→IP offline CLI) is ✅ accepted** (see [owner-transfer.md](owner-transfer.md)); the supplied complete real PG16+age `pnpm verify:release` success evidence includes the real PostgreSQL owner-transfer gate, the real age gate, and the compiled + installed-npm PostgreSQL E2E smoke, all passing; no test counts are recorded. This applies to WP5D only: WP5 as a whole remains incomplete, WP5B is DEFERRED, and the WP5C deployment drill is not accepted.

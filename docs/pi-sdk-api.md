...
- 若 loader 的 `cwd` 跟着项目 cwd 交替（startup 探针用服务 cwd、项目探针用项目 cwd、冻结会话又回服务 cwd），`useExtensionCacheCwd()` 每次遇到不同 cwd 就 `clearExtensionCache()`，扩展模块整进程被反复重新求值；模块级副作用（例如扩展在 `globalThis.fetch` 上再包一层）会层层叠加，进程生命周期内无法卸载。

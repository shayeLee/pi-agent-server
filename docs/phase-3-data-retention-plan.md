...
> 本文是 Phase 3 的**唯一状态台账**与计划文档：工作包状态、已确认决策、目标行为与当前代码的差距（待实现）集中于此，其他文档不复制状态。
...
- **RC 阶段**：服务级正式 migration / 备份 / 回滚**未接入服务启动**；`pnpm migrate`/backup/restore/cutover/file-ops/reconcile-jsonl 均为离线开发期工具。
- 当前开发用 SQLite 目标无旧 RC 数据，已通过空库 pre-migration 加密备份 + migration apply + verify 建立干净基线；**破坏性 reset/cutover 从未对真实用户数据执行**，当前也没有需要 cutover 的目标。
- 不执行真实用户数据备份/恢复演练，不部署任何备份任务；启动 migration 门禁目标行为（§4）与 backup/restore 目标语义（§5）**待实现**。
...
| 目标（计划） | 当前代码 | 状态 |
| --- | --- | --- |
| 新增独立数据模式 `managed`/`rc`（配置名在实现时冻结，目标默认 `managed`）；`PI_MIGRATION_GATE` 仍为 `off`/`verify`，**目标默认 `verify`**；`managed` 必须搭配 `verify`，且**绝不自动迁移** | 当前没有数据模式；`PI_MIGRATION_GATE` 默认 `off`，`verify` 为 opt-in 只读 ledger/head 校验 | **待实现** |
| backup 对缺失 session 引用按 **missing-as-empty** 处理并**允许发布**（不 fail） | 默认运行把缺失引用记入 manifest 并发布；strict 模式（`--require-complete-session-references`）与 owner-transfer always-strict 使缺失引用 fail-closed | **待实现**（目标默认 missing-as-empty 允许发布） |
| backup **不做 JSONL 语义校验**（不逐行 JSON.parse） | backup 逐行 JSON.parse 校验 | **待实现** |
| restore 时对无效 JSONL 按 **invalid-as-empty** 处理 | 当前 restore 校验 JSON/JSONL 有效性（行为见 [backup-restore.md](backup-restore.md)） | **待实现** |

## 4. Migration 引擎与启动门禁

- **引擎（已确认选型）**：自定义 Manifest-driven，以运行时 Schema Manifest 为 head 真相；ledger（`schema_migrations`）与业务表同库同事务写入；稳定 checksum（内容即代码，已应用版本 checksum 改变 fail-fast）；迁移锁按可替换实现（单实例进程内，PG advisory lock 为多实例形态）；SQLite `BEGIN IMMEDIATE` / PG 显式事务内执行，失败回滚；**空库**→依序应用、**版本 < head**→升级、**版本 > head**→fail-fast（禁降级）、**版本 = head 但 checksum 不符**→fail-fast；既有 RC 未迁移库拒绝自动 baseline adoption。详见 [database-design.md](database-design.md)。
- **不可变**：已发布 migration 不得修改，只能新增后续版本；生产回滚**靠备份恢复、不自动 down**（双存储非单事务，自动 down 破坏一致性）。
- **启动门禁**：当前为可选 opt-in（`PI_MIGRATION_GATE=verify`，只读校验、默认 `off`）。目标是新增独立的 `managed`/`rc` 数据模式并把 gate 默认改为 `verify`；`managed + off` 必须在创建资源前 fail-fast，`rc + off` 仅供显式 disposable RC 使用，任何模式都不自动迁移（**待实现**）。readiness 呈现面已由 WP5A 交付。

## 5. 备份与恢复

- **备份集** = DB 快照 + 当次实际存在的 JSONL 会话文件。缺失引用按已确认业务语义视为无历史，记录后仍允许发布；DB 快照与 JSONL 复制不是全局原子操作，必须稳定复制并校验包级 hash/size 后才发布。
- **实现形态**：SQLite 用 `VACUUM INTO` 一致性快照 + JSONL 点快照；PostgreSQL 用 `pg_dump` + JSONL 点快照。JSONL 在 backup 阶段只作为 opaque bytes，不做内容合法性判断；age 加密、逐项 hash/size、`COMPLETE` 最后写入。上述目标语义仍待实现，当前差异见 §3 和 [backup-restore.md](backup-restore.md)。
...
| WP2 | ✅ 当前目标不适用 | 受控 cutover 工具已实现；实际 reset/cutover 从未执行。当前开发目标无旧 RC 数据且已建立干净基线，因此没有 cutover 目标；未来若出现 legacy disposable RC 目标，必须重新授权 | [cutover-runbook.md](cutover-runbook.md) |
...
- WP2A 受控 cutover runbook：[cutover-runbook.md](cutover-runbook.md)

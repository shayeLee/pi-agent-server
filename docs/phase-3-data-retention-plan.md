...
| Phase 3（WP2 及以后） | 🟡 进行中（WP3 基础工作包已完成；WP2A 受控 cutover 实现与干净 SQLite 基线初始化已完成；当前目标无旧 RC 数据，破坏性 WP2B reset 不适用；WP4A、WP4B 方案 A 只读 planner 与 WP4C 方案 A 收敛 DB-only reconcile analyzer 均已验收；**WP5A 最小运维门禁基础（/readyz、/metrics、启动 migration-gate readiness 契约）✅ 已验收——仅进程 readiness/metrics，不改变 request idempotency 与 shutdown persistence；WP5B（durable idempotency/shutdown persistence 加固）按用户决定 DEFERRED、且不算完成；WP5C 方案 B 部署契约已形成、可评审、未经过实际部署演练不验收；自动 retention 属未来工作包（未开始、未排期）；WP6 尚未开始**，正式常驻服务未启用） |
...
- **WP5A 已验收，交付最小运维门禁基础（/readyz、/metrics、启动 migration-gate readiness 契约）**：无 backup scheduler/timer、无 retention、无备份缺失/过期 age alerts、无 RPO/RTO 默认阈值、无 restore drill 调度；/readyz 与 /metrics 不等价于备份新鲜度或生产就绪；**不改变 request idempotency 与 shutdown persistence**（durable idempotency/shutdown-persistence 加固计划为 WP5B——按用户决定 DEFERRED、且不算完成；备份新鲜度告警由 WP5C 方案 B 部署契约覆盖（已形成、可评审、未经过实际部署演练不验收——见 §8.1），自动 retention 属未来工作包）；
...
- **已确认方向**：每日完整备份；每日备份在线执行，**不要求每日停服务**；每次 migration 前强制备份；定期恢复演练（已确认：每季度 + 每次重大 migration 前）。「每日完整备份」即当前承诺粒度（增量/差异备份留给未来，不承诺）。
- **备份保留期（已确认）**：备份保留 **30 天**。**但当前不自动删除（未实现、未排期）**：30 天保留期作为运维策略记录，过期备份由运维人工判断后手动清理；自动 retention worker 属于**未来单独工作包**（明确不属于 WP5B（durable idempotency 加固）也不属于 WP5C（exporter 部署契约）范围）。
- **备份调度（已确认）**：备份由**部署方经过审核的 helper/timer**（或等效 OS 调度器；本代码库不交付/不安装任何 helper、timer、unit 或 plist）驱动，以**固定 12h 节奏（≤ 12h；§6.5 默认节奏，不提供 24h 示例）**调用**固定构建产物（整个 `dist-backup` 运行时传递闭包及其祖先链 root-owned、非 symlink、无 group/world 写；绝对 root-owned 编译产物 backup CLI + 确切 pinned node ≥ 22.19；非 `pnpm backup`，非 `AGENT_CWD`；`age`/`age-keygen`/`pg_dump`/`pg_restore` 仅用已审核绝对路径或受控 root-owned 安全 PATH，并由 helper 验证 resolved binary/版本）**——**`pnpm backup` 仅作为人工 dev 命令**；**不使用服务进程内置 timer/scheduler**。timer 配置在部署层管理并审核，不在本代码库中安装；自动化叙述统一为「部署方经过审核的 helper/timer（见契约）」。对自动化的监控（备份是否按时执行）由 **WP5C 方案 B backup freshness 部署契约** 定义（per-target textfile 指标仅在 backup CLI 已验证 published 完成后更新、失败绝不更新 + PromQL 规则 + 外部 Alertmanager；契约已形成、可评审、未经过实际部署演练不验收，见 §8.1 WP5C）覆盖。
...
- **定期（已确认：每季度 + 每次重大 migration 前）**在隔离/临时实例执行：加载备份 → 启动门禁（preflight + ledger 校验）→ 抽查会话数据完整性 → 产出演练报告。默认 restore drill 只记录 restore-time/完整性证据，不能验收 RTO=4h；完整 RTO signoff 必须另行授权，在隔离 target-like 环境以合成且无敏感业务数据计时 `restore → PI_MIGRATION_GATE=verify 启动 → /health、/readyz 与合成无敏感业务检查 → 可服务`，随后关闭并清理隔离服务，且全流程 `<=4h`。WP3B2 的 PG 演练使用运维/测试 fixture 预先创建和最终销毁临时 database/schema；core 只在空 target DB 上执行 `pg_restore`，不默默创建、drop 真实数据库。migration prebackup、生产回滚门禁仍留给后续工作包。
- 备份「有效」的唯一证明是恢复演练通过；演练同样覆盖 §4.8 的迁移出错回滚。

### 6.5 RPO / RTO（已确认）

- **RPO（Recovery Point Objective）= 24h**：固定 12h 完整在线备份为粒度（默认节奏，契约不提供每日 24h 示例）；最大数据丢失窗口为上一次成功备份到故障点之间的时间。备份由部署方经过审核的 helper/timer 驱动、调用固定构建产物 backup CLI（见 §2.2），不依赖服务进程自身。
- **RTO（Recovery Time Objective）= 4h**：默认 restore 部分只产出 restore-time/完整性证据，不能单独验收或签署 4h RTO。完整 signoff 必须在隔离 target-like 环境、仅使用合成且无敏感业务数据，计时 `restore → PI_MIGRATION_GATE=verify 启动 → /health、/readyz 与合成无敏感业务检查 → 可服务`，结束后关闭并清理隔离服务；执行需授权且不得正式服务。
- **验收口径**：RPO 由备份 freshness 告警（§8.1 WP5C 方案 B 部署契约的 PromQL 规则 + 外部 Alertmanager）覆盖——外部 Alertmanager 基于 Prometheus 备份时间指标判定是否超 24h 阈值；RTO 只有在上述完整流程全程 `<=4h` 且有阶段时间证据时才可签署，默认 restore drill 报告不构成 RTO signoff。
- **监控架构（已确认）**：Prometheus 指标由 **WP5C 方案 B backup freshness 部署契约** 定义——部署方经过审核的 helper/timer 以**固定 ≤ 12h 节奏**调度固定构建产物 backup CLI（整个 `dist-backup` 运行时传递闭包及其祖先链 root-owned、非 symlink、无 group/world 写；绝对 root-owned 编译产物 + 确切 pinned node ≥ 22.19；非 `pnpm backup`，`pnpm backup` 仅人工 dev；`age`/`age-keygen`/`pg_dump`/`pg_restore` 仅用已审核绝对路径或受控 root-owned 安全 PATH，并由 helper 验证 resolved binary/版本）；**secret 不得 argv**（受限 root:root 0600 env 配置，严格单一 `KEY=VALUE` 语义，经进程环境传递——任何 argv/unit/plist/日志无 secret）；**服务 auth token 不可读取**（`PI_AUTH_PATH` 服务账号属主 0600，backup 用户不可读内容，仅持每个祖先目录精确 traverse（仅 search）ACL，Linux setfacl / macOS chmod +a 或等价机制——无 root preflight 替代）；**per-target** node_exporter textfile 指标 `pi_agent_server_backup_last_success_timestamp_seconds`（值 = 调用前记录的 **conservative backup start**，wall-clock epoch）**仅在 backup CLI 已验证 published 完成后更新**（exit 0 + 机器可读 published output 校验通过、拒绝 dry-run），**失败绝不更新**（指标停留在上次成功时间）；**target root / ACL / atomicity 由部署审核**（textfile 目录 root 属主 0750、node_exporter 组只读、完整祖先链 root-owned/非 symlink/无 group/world 写；原子替换由部署方在目标 OS 审核留证——仓库不宣称跨 OS 原子发布或 fd safety）；Prometheus 规则统一按**独立持久 inventory 指标 `pi_agent_server_backup_expected_target_info{job,cluster,instance}=1` 与完整 (job, cluster, instance) actual 目标三元组** 过滤，含 missing（精确为 `(I and up==1) unless F`；`I unless A` 仅用于 Q3 漂移，不使用全局 `absent()`）/ stale（`time() - metric > 24h` 覆盖 RPO）/ future-timestamp / exporter-down / scrape-error 规则与唯一性验收查询（Q1–Q3：完整三元组精确计数、instance 跨 job/cluster 重复、inventory/actual 双向集合）。契约见 [backup-freshness-exporter.md](backup-freshness-exporter.md)，**已形成、可评审、未经过实际部署演练不验收**（方案 A 仓库内 scanner 已放弃，未来 native/identity 方案另议）+ **外部 Alertmanager**（非本代码库组件）消费 Prometheus 指标并触发告警。本代码库不安装 Alertmanager、不交付任何 helper/timer/unit/plist 或运行脚本。
- **当前状态**：RPO/RTO 数值已确认，备份保留期 30 天已确认，timer 调度策略已确认，监控架构已确认。但 WP5C 方案 B 仅有已形成、可评审的部署契约（未经过实际部署演练不验收；方案 A scanner 已放弃；WP5B 按用户决定 DEFERRED、且不算完成；自动 retention 未排期），因此 RPO/RTO 尚无自动化验收闭环；数值作为运维目标承诺记录于此。
...
- **Strict backup completeness foundation：✅ 已验收。** 用户提供的修复 fixture 后完整真实 PG16+age `verify:release` 成功证据包含 strict completeness compiled/npm 门禁及真实 PostgreSQL CLI gate 通过；本文不记录测试数量。此次仅验收 backup core/CLI 基础，不改变 WP5C 方案 B 的状态：WP5C 仍为已形成、可评审、未验收，必须实际演练 helper/timer → textfile → Prometheus → Alertmanager 后才能验收。
- 当前策略对缺失但位于白名单根内的 `pi_session_file` 做加密 manifest 状态记录；白名单根外引用直接失败。**另有 opt-in strict 完整性模式**（CLI `--require-complete-session-references`，精确拼写、重复/未知参数 fail-closed）：任一缺失引用在**任何 final publish/COMPLETE 之前** fail-closed（非零退出、清理 staging、错误稳定脱敏只含计数、不泄 path/ref）；自动化 freshness 只承认 **strict + 已发布成功**（机器行 `backup-json-report:`，status=published/strict=true/dryRun=false/missingSessionReferences=0），dry-run 与人工默认运行明确不构成 freshness。PostgreSQL backup 由 WP3B2 提供实现且 reviewer P0/P1 修复已完成，真实 PG16 `pg_dump` → age → `pg_restore` gate 已通过；retention、migration-prebackup/runbook 与外部 scheduler 接入留给后续工作包；外部 scheduler 只能在运维层显式调用 CLI。
...
| WP5 | 🟡 进行中但**未验收**（**WP5A ✅ 已验收；WP5B 按用户决定 DEFERRED、且不算完成；WP5C 方案 B 部署契约与实际部署演练 SOP 已形成、演练按用户决定 deferred、未验收，方案 A scanner 已放弃**） | 运维门禁 | WP5A（✅）：/readyz + /metrics + 启动 migration-gate readiness；WP5B（DEFERRED）：durable request-idempotency/shutdown-persistence 加固（不算完成）；WP5C（方案 B，契约已形成、可评审、未经过实际部署演练不验收）：backup freshness 部署契约——部署方经过审核的 helper/timer 以固定 ≤ 12h 节奏调用固定构建产物 backup CLI（整个 `dist-backup` 运行时传递闭包及其祖先链 root-owned、非 symlink、无 group/world 写；确切 pinned node ≥ 22.19；非 AGENT_CWD/pnpm），`age`/`age-keygen`/`pg_dump`/`pg_restore` 仅用已审核绝对路径或受控 root-owned 安全 PATH 且由 helper 验证 resolved binary/版本，secret 不得 argv、服务 auth token 不可读取（仅精确 traverse ACL、无 root preflight 替代）；**per-target textfile 指标（`pi_agent_server_backup_last_success_timestamp_seconds`，conservative backup start）仅在 backup CLI 已验证 published 完成后更新（exit 0 + 机器可读 published output 校验、拒绝 dry-run），失败绝不更新**；target root/ACL/atomicity 由部署审核（textfile 目录 root 属主 0750、node_exporter 组只读、完整祖先链 root-owned/非 symlink/无 group/world 写；不宣称跨 OS 原子发布）；Prometheus 规则以**独立持久 inventory `pi_agent_server_backup_expected_target_info{job,cluster,instance}=1`**为 expected 清单，并按完整 `(job, cluster, instance)` 三元组匹配 actual freshness/up/textfile，验证唯一性，含 missing（精确为 `(I and up==1) unless F`；`I unless A` 仅用于 Q3 漂移，不使用全局 absent）/ stale（>24h）/ future / exporter-down / scrape-error 规则与唯一性验收查询（完整三元组精确计数、instance 跨 job/cluster 重复、inventory/actual 双向集合）；**仓库不交付/不安装任何 helper、timer、unit、plist 或运行脚本——自动化一律为「部署方经过审核的 helper/timer（见契约）」，无 pnpm/CLI 自动 timer 示例**；无扫描（方案 A 已放弃，hygiene 回归断言）、无 age identity、无服务集成、无自动 backup、无删除；自动 retention（30 天）＝未来单独工作包；未来 native/identity 方案另议 | WP1、WP3B | WP5A：已验收；WP5B/WP5C：SOP 与门禁/告警清单已落文档；实际部署演练按用户决定 deferred；当前无 WP5C 验收证据，WP5C/WP5 未验收 | 门禁规则配置化，可降级为告警（需决策） |
...
**WP5B（按用户决定 DEFERRED，且不算完成——不要实现）**：WP5B 仅承接 **durable request-idempotency / shutdown-persistence 持久加固**（如 durable processing marker、终态持久化有界化等）。当前仅有**进程内 in-flight 去重 + 持久化终态读取**；若在终态落库前崩溃，相同 `requestId` 可能再次执行；不承诺 exactly-once 或 durable at-most-once。仅在 side-effect tools 正式启用、多实例、公开服务或明确严格防重放要求时重新触发 WP5B。**备份缺失/过期告警不属于 WP5B**——由 WP5C 方案 B 部署契约的 PromQL 规则 + 外部 Alertmanager 覆盖（契约已形成、可评审、未经过实际部署演练不验收）；**审计保留衔接与自动 retention（30 天自动清理）也不属于 WP5B/WP5C**，属未来单独工作包（未开始、未排期）。**本计划仅记录 WP5B 的延期决策；WP5B 不算完成**，WP5A 的验收范围也不承诺其中任何行为。

**WP5C（方案 B：backup freshness 部署契约——契约已形成、可评审、未经过实际部署演练不验收；方案 A 已放弃）**：WP5C 方案 B **不在仓库内写任何扫描/observer 代码、CLI、tests 或构建产物**（`dist-hygiene.mjs` 已禁止方案 A scanner 的编译产物 `health-core.*` 回归），**不交付/不展示任何可复制的 root Node helper 源码、shell 运行脚本、systemd unit 或 launchd plist，不声称跨 OS 原子发布实现**；交付物为**部署契约 + 验收清单**（见 [backup-freshness-exporter.md](backup-freshness-exporter.md)），自动化入口一律为「**部署方经过审核的 helper/timer（见契约）**」，禁止直接 pnpm/CLI 自动 timer 示例（`pnpm backup` 仅人工 dev）。契约要求：**固定构建产物**（整个 `dist-backup` 运行时传递闭包及其祖先链 root-owned、非 symlink、无 group/world 写；绝对 root-owned 编译产物 backup CLI + 确切 pinned node ≥ 22.19，非 AGENT_CWD/pnpm；`age`/`age-keygen`/`pg_dump`/`pg_restore` 仅用已审核绝对路径或受控 root-owned 安全 PATH，并由 helper 验证 resolved binary/版本）；**调度节奏 ≤ 12h**（默认固定 12h，不提供每日 24h 示例，随机延迟 ≤ 300s 计入预算）；**secret 不得 argv**（受限 root:root 0600 env 配置、严格单一 `KEY=VALUE` 语义——零注释/引号/展开/重复/CR；secret 只经进程环境传递，任何 argv/unit/plist/日志无 secret 值）；**服务 auth token 不可读取**（`PI_AUTH_PATH` 服务账号属主 0600、backup 用户不可读内容，仅持每个祖先目录精确 traverse（仅 search、不可 list/写）ACL——Linux setfacl / macOS chmod +a 或等价机制，无 root preflight 替代）；**per-target textfile 指标仅在 backup CLI 已验证 published 完成后更新**（exit 0 + 机器可读 published output 校验通过、拒绝 dry-run；**自动化必须带 strict flag `--require-complete-session-references`**——机器契约 = 单行 `backup-json-report:` JSON 报告（status=published、strict=true、dryRun=false、missingSessionReferences=0），strict 成功即声明引用完整，任一缺失 session reference 在发布/COMPLETE 前 fail-closed、绝不更新指标；发布路径位于 BACKUP_ROOT 下且 owner=backup 用户；值 = 调用前记录的 **conservative backup start**，wall-clock epoch、时钟同步要求见契约 §5），**失败绝不更新**（指标停留在上次成功时间 → stale 规则自然触发）；从未成功 → 指标缺失 → 由独立持久 inventory `pi_agent_server_backup_expected_target_info{job,cluster,instance}=1` 与 actual freshness 的 per-target `unless` 触发缺失告警（不使用全局 `absent()`）；Prometheus 规则以独立持久 inventory `pi_agent_server_backup_expected_target_info{job,cluster,instance}=1` 为 expected 清单，按完整 `(job, cluster, instance)` 三元组匹配 actual freshness/up/textfile 并验证唯一性（Q1–Q3 验收查询），`time() - metric > 24h` 覆盖 RPO，另有 **node_exporter down（`up==0`）、`node_textfile_scrape_error==1`、future-timestamp** 告警；**target root / ACL / atomicity 由部署审核**（backup root 由 backup 用户属主 0700、父目录 root:root 0755；textfile 目录 `root:node_exporter 0750` 组只读、完整祖先链 root-owned/非 symlink/无 group/world 写；原子替换由部署方在目标 OS 审核留证——不宣称跨 OS 原子发布或 fd safety）；Alertmanager 由外部配置。30 天保留人工清理 + 每季度/重大 migration drill 人工流程（major migration drill 在窗口前用最近 drill backup 完成，正式 pre-migration apply 不停顿）。**明确不做**：无扫描、无 age identity、无服务集成、无仓库 timer/helper、无自动 backup、无删除。**方案 A（仓库内只读 backup health scanner observer）已放弃；未来 native in-process metrics 或基于 age identity 的方案另议（单独事项）。未验收（可评审）：未经过实际部署演练不验收；本文不记录或推导测试数量。**
...
WP0 决策冻结
 └─→ WP1 迁移引擎 ──→ WP3A 备份核心 ──→ WP3B1 SQLite drill ──→ WP3B 后续恢复/门禁 ──→ WP2 最终 reset 切换（== 数据保留起点）
     ...
     └─→ WP5 运维门禁 ──（WP5A 最小运维门禁基础：/readyz、/metrics、启动 migration-gate readiness 契约 — ✅ 已验收；WP5B：durable idempotency/shutdown persistence 加固 — DEFERRED、且不算完成；WP5C：backup freshness 方案 B 部署契约已形成、可评审、未经过实际部署演练不验收（仓库不交付/不安装 helper/timer——自动化由部署方经过审核的 helper/timer 承担、无 pnpm/CLI 自动 timer 示例；不自动 backup/无扫描——方案 A scanner 已放弃；自动 retention 属未来工作包；未来 native/identity 另议））└─→ WP6 IAM 进入条件（仅当 WP1–5 全部通过）
```

- **关键门槛**：只有 WP1–5 全部通过，才能进入 WP6（IAM 数据落地）——WP3 基础工作包（WP3A/WP3B1/WP3B2/WP3C）已完成各自真实 gate；WP4A 已验收，**WP4B 方案 A 的安全只读 planner 已验收（物理 executor，包括 unlink、retry/quarantine 未实施）**，WP4C 方案 A 收敛 DB-only reconcile analyzer ✅ 已验收（绝不扫描文件系统，不能探测 orphan/lost/JSONL 损坏，自动处置未实施），**WP5A（最小运维门禁基础：/readyz、/metrics、启动 migration-gate readiness 契约）✅ 已验收——仅进程 readiness/metrics，不改变 request idempotency 与 shutdown persistence；WP5B（durable idempotency/shutdown persistence 加固）按用户决定 DEFERRED、且不算完成，备份新鲜度告警由 WP5C 方案 B 部署契约（已形成、可评审、未经过实际部署演练不验收）覆盖，自动 retention 属未来工作包（未开始、未排期）；WP6 尚未开始**。WP2A 工具已实现并通过真实 PG16+age `verify:release` 门禁；当前 SQLite 目标已完成干净基线初始化，破坏性 WP2B reset 不适用。
...
| 2 | ~~RPO / RTO 取值~~ | WP3 | **已决策（§6.5）：RPO 24h，RTO 4h；RPO 由 Prometheus backup freshness 告警（§8.1 WP5C 方案 B 部署契约的 PromQL 规则 + 外部 Alertmanager）覆盖；RTO 仅由获授权的隔离 target-like 全流程计时 signoff（restore → `PI_MIGRATION_GATE=verify` 启动 → `/health`/`/readyz` + 合成无敏感业务检查 → 可服务，随后关闭清理）覆盖，默认 restore-time 报告不构成 RTO signoff；自动化验收闭环尚未实现** |
| 3 | ~~备份保留期（部分）~~ | WP3/WP4/WP5 | **已决策：备份保留 30 天；自动删除尚未实现、未排期（未来单独工作包，不属于 WP5B/WP5C）**；会话 TTL 归档/删除参数、审计保留期、quarantine 保留期仍待定 |

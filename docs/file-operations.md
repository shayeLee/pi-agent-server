...
- **显式 CLI** `scripts/file-ops.ts`（bin `pi-agent-server-file-ops`）：只支持默认 / `--dry-run` 只读计划；SQLite 以 `readOnly` 打开（缺失 DB 绝不创建 DB/WAL/SHM），PostgreSQL 连接强制 `default_transaction_read_only=on`；
- **fail-closed `--apply`**：物理执行器未实施，`--apply` 立即以退出码 2 拒绝，零 claim/lease/complete/fail/文件操作；不存在任何确认词/维护窗口词可以绕过。

绝不：claim/lease/complete/fail；扫描文件系统生成新操作（WP4C reconcile 的职责）；自动启动 worker/timer；由 HTTP 请求驱动；把相对/绝对路径写进报告或错误输出。
...
- 连接规则：SQLite 需要显式绝对 `DB_PATH`，以 `readOnly: true` 打开——文件不存在时直接失败（**绝不创建 DB/WAL/SHM**），已存在的库保持字节指纹不变；PG 需要显式 `PI_STORAGE_DIALECT=postgres` + `PI_DATABASE_URL`，连接串强制追加 `default_transaction_read_only=on`（原 URL 已含 `options` 参数时拒绝合并，fail-closed）；
...
- `pnpm test:file-ops-pg`：**强制真实 PG planner 门禁**（无 `PI_TEST_PG_URL` 非零失败），在随机专用 schema 上跑只读计划用例：零写、无路径泄漏、readOnly URL 同时保留随机 schema `search_path` 与 `default_transaction_read_only=on`（`SHOW` 双断言）且写操作被服务端拒绝；**门禁实际运行 source CLI PG 分支**（`scripts/file-ops.ts` 经 tsx，fixture 创建的随机 role 通过 `ALTER ROLE … SET search_path` 绑定随机 schema，CLI 强制追加只读约束），验证随机 schema 隔离、只读、零 DB 变化、stdout/stderr 无 URL/path/credential 泄漏）；该门禁已包含在用户提供的成功 `verify:release` 证据中。

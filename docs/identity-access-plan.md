...
>
> **WP5D 状态注记（当前 RC）**：§2 描述的「静态 `TOKENS` + 内网来源 IP 免 token」RC 机制已被 **WP5D-2 网络准入**取代并废弃（`INTRANET_CIDRS`/`TOKENS`/`TRUST_PROXY` 设置即拒绝启动）：现在所有路由以直接 TCP 对端 IP 过 CIDR/disabled gate，`/v1` 与 `/metrics` 上策略画像 `tokenRequired` 才要求 Bearer token（hash 绑定精确 IP；`/health`、`/readyz` 永不需要 token）。**WP5D-1 policy core、WP5D-2 网络准入与 WP5D-3 role 授权矩阵 ✅ 已验收**（依据用户提供的在移除 `workspaceRoots`/`PI_DEFAULT_WORKSPACE_ROOT` 并提交 `5e84a9da` 之后的新 release run 中，完整真实 PG16+age `pnpm verify:release` 成功证据；本次验收仅针对 WP5D，不代表 WP5 整体完成；WP5B 仍 DEFERRED，WP5C deployment drill 仍未验收）：基于 `request.access.role` 的逐路由矩阵（探针任意 admitted role、`/metrics` 仅 admin/operator（画像 tokenRequired 时实际 GET 仍须 token）、operator 的 `/v1` 一律 403、viewer 仅纯读 GET/export/SSE（export 零实例化、viewer SSE 无 runtime 稳定 204）、user/admin 维持 own-resource 行为仍 owner 隔离）。**当前 RC 用户决策收窄 WP5D：`workspaceRoots`/`PI_DEFAULT_WORKSPACE_ROOT` 整体移除**（内网不做 workspace 强制；workspace/sandbox 安全仅公网暴露前需要，延期至未来 OIDC/IAM + workspace/sandbox 设计；IP-RBAC 不限制 cwd 或 Agent 工具绝对路径/OS 权限，不是 sandbox），WP5D 当前状态为 **✅ 已验收**——admin 跨 owner 只读未实现、**WP5D-4 owner transfer（DB 层 IP→IP）✅ 已验收**（见 [owner-transfer.md](owner-transfer.md)）：用户提供的完整真实 PG16+age `pnpm verify:release` 成功证据中，真实 PostgreSQL owner-transfer gate、真实 age gate，以及 compiled + installed-npm PostgreSQL E2E smoke 均通过；无 DB schema 变更。本文作为 IAM 路线图的历史基线保留原始表述；当前准入与授权语义见 [ip-rbac-design.md](ip-rbac-design.md)。本文不记录测试数量。
>
> **数据迁移语义（RC 决策，实现按此边界）**：新 RC **不存在 legacy 账号/token 的 owner 迁移**——正式旧公网 token 数据从未存在，因此不实现任何「旧 token/旧账号 → 新主体」迁移代码。早期开发库按 RC 语义**删库重建 / 经受控离线 cutover（`pnpm cutover`）reset**，绝不在位转换（见 §7 工作包 0 数据策略冻结与 [database-design.md](database-design.md) §7）。**WP5D-4 owner transfer 已验收（✅，见 [owner-transfer.md](owner-transfer.md)）**，用户提供的完整真实 PG16+age `pnpm verify:release` 成功证据中，真实 PostgreSQL owner-transfer gate、真实 age gate，以及 compiled + installed-npm PostgreSQL E2E smoke 均通过；其范围仅限 DB 层把资源归属在两个 canonical IP 身份之间转移（**IP→IP**），**不迁移策略文件的 IP 条目、token 绑定与角色**——接收方继承自己的 IP 画像，与资源原 owner 的画像无关（见 [ip-rbac-design.md](ip-rbac-design.md) §7）。

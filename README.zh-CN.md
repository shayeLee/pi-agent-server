...
- **PG/age 状态** —— 仅当前环境实际运行真实 gate 才可验收：PG 需要 `PI_TEST_PG_URL`，age gate 需要 `age`/`age-keygen`。缺少前置条件时结果是 skip 或 fail-closed，绝不称为“通过”。WP4A 的 storage/outbox/restore 加固已依据用户提供的真实 PG16+age gate 成功证据验收；WP4B 的安全只读 planner 已依据该证据验收（物理 executor，包括 unlink、retry/quarantine **未实施**）；WP4C（reconcile）与 WP5/WP6 尚未开始。**尚非生产就绪**：服务级 migration、备份、回滚与生命周期接入尚未实施，仅限 RC 阶段使用。
...
pnpm verify             # 日常门禁：typecheck + test + build:backup + build:file-ops
...
pnpm verify:release     # 完整发布门禁：typecheck + test + test:postgres + test:pg-backup + test:migration-prebackup + test:cutover + test:cutover-pg + test:file-ops-pg + test:age（由 test:restore-real 调用）+ test:restore-real + build + build:migrate + build:backup + build:cutover + build:file-ops（需要 PI_TEST_PG_URL 与全部真实二进制）
...
- **发布门禁（P0）：** 不能在没有运行 typecheck、PG 测试被 skip、真实 PG backup 门禁被 skip、真实 cutover 演练门禁被 skip、真实 WP4B planner PG 门禁被 skip 或真实 age restore 门禁被 skip 时宣称完整验收。`pnpm verify:release` 依次执行 `typecheck` + `test` + `test:postgres` + `test:pg-backup` + `test:migration-prebackup` + `test:cutover` + `test:cutover-pg` + `test:file-ops-pg` + `test:age`（由 `test:restore-real` 调用）+ `test:restore-real` + `build` + `build:migrate` + `build:backup` + `build:cutover` + `build:file-ops`；`release:rc` 发布前调用 `verify:release`。日常循环用 `pnpm verify`（typecheck + test + build:backup + build:file-ops，无需数据库）。

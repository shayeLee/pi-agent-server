...
function assertRestoreReport(targetRoot, result) {
  ...
  if (!row || realpathSync(row.conversation_ref) !== realpathSync(expected) || row.conversation_ref.includes(path.join(dataDir, "sessions")) || !readFileSync(expected, "utf8").includes('"type":"session"')) throw new Error("compiled restore data/remap verification failed");
}
...
try {
  ...
  if (missingPublished.stdout.includes("missing-session") || missingPublished.stdout.includes(path.join("projects", "p", "sessions", "missing-session"))) throw new Error("compiled backup leaked a reference/path on the missing path");

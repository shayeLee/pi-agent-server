...
async function main(): Promise<void> {
  ...
  if (storage.dialect === "postgres") {
    ...
    else console.log(`backup created: ${result.finalPath}; ${result.files.length} encrypted payload(s), ${result.missingSessionReferences.length} missing session reference(s)`);
    return;
  }

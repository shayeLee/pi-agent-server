...
### Trusted-plugin `runTurn` turn budgets

`PluginSessionApi.runTurn` is synchronous and plugin-owned, so the host enforces hard per-turn budgets instead of letting a pathological turn run until the caller times out. Three budgets are host-enforced and not adjustable by plugins: returned assistant text (`PLUGIN_RUN_TURN_LIMITS.maxAssistantTextLength`), total tool calls (`PLUGIN_RUN_TURN_LIMITS.maxToolCallsPerTurn`, default 60; successful and failed calls both count), and wall-clock duration (`PLUGIN_RUN_TURN_LIMITS.maxTurnDurationMs`, default 5 minutes). When any budget is exceeded the host aborts that turn quickly and returns `{ status: "error", message, code }`, where `code` is one of the stable values `turn_tool_budget_exceeded`, `turn_duration_budget_exceeded`, or `turn_assistant_text_budget_exceeded`; the `code` field is additive, so existing consumers that only read `status`/`message` keep working. Budgets are `runTurn`-only: ordinary chat turns (`POST /v1/sessions/:id/messages`) are unaffected and still have no tool-call or wall-clock budget. The duration budget uses a real timer that is cleared on settle/dispose, so it also catches a fully silent hang that emits no events. These codes have a single authoritative definition in the host (`TURN_ERROR_CODES`): it is exported statically via `pi-agent-server/contract` for same-machine consumers and injected into plugins through `PluginHostContext.turnErrorCodes` (plugins are loaded by absolute path and cannot statically import the host package), so plugins no longer keep a copy that could drift.

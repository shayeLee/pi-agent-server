...
| `GET` | `/readyz` | Readiness (unauthenticated; WP5A ✅ accepted: this process's startup + migration gate only — see [Ops probes](#ops-probes-readyz-and-metrics-wp5a-accepted)) |
| `GET` | `/metrics` | Prometheus text exposition (unauthenticated; WP5A ✅ accepted fixed small surface — see [Ops probes](#ops-probes-readyz-and-metrics-wp5a-accepted)) |
...
Two unauthenticated probes are served beside `/health` for operator monitoring:

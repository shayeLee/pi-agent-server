...
it("HTTP DELETE session/project only enqueues file cleanup and never unlinks", async () => {
  ...
  try {
    ...
    await storage.sessions.create({
      ...
      createdAt: 1, updatedAt: 1, conversationRef: file, modelProvider: null, modelId: null,
    ...
    expect(await storage.fileOperations.list()).toEqual([
      expect.objectContaining({ operationKey: artifactDeleteOperationKey("pi", "pi-jsonl-v3", "sessions/http-session/history.jsonl"), relativePath: "sessions/http-session/history.jsonl", state: "pending" }),
    ]);
    ...
    await storage.sessions.create({
      ...
      agentKind: "pi",
      conversationFormat: "pi-jsonl-v3",
      conversationRef: projectFile, modelProvider: null, modelId: null, thinkingLevel: null,
    ...
    expect(await storage.fileOperations.getByOperationKey(artifactDeleteOperationKey("pi", "pi-jsonl-v3", "projects/http-project/sessions/http-project-session/history.jsonl"))).toMatchObject({
